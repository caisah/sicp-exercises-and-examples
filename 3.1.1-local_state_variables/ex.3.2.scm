...
;; times a given procedure is called during the course of a computation. Write a procedure
...
;; the number of times it has been called by maintaining an internal counter. If input
;; to mf is the special symbol how many calls?, then mf returns the value of the counter.
;; If the input is the special symbol reset-count, the mf resets the counter to zero.
  ...
  (define (mf m)
    (cond ((eq? m 'how-many-calls) counter)
          ...
          ((number? m)
           ...
           (f m))
          (else
           (error "Unknown request - MAKE-MONITORED"
                  m))))
  mf)
